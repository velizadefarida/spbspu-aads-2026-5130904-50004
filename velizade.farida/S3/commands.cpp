...
void velizade::cmdOutbound(std::istream& in, std::ostream& out)
{
    ...
    std::map<std::string, Vector<int>> outMap;
    for (auto it = g.edges.begin(); it != g.edges.end(); ++it)
    {
        auto kv = *it;
        if (kv.first.first == vertex)
        {
            Vector<int> weights = kv.second;
            std::sort(weights.begin(), weights.end());
            outMap[kv.first.second] = weights;
        }
    }
    for (auto& p : outMap)
    {
        out << p.first;
        for (size_t i = 0; i < p.second.getSize(); ++i)
        {
            out << " " << p.second[i];
        }
        out << "\n";
...
void velizade::cmdInbound(std::istream& in, std::ostream& out)
{
    ...
    std::map<std::string, Vector<int>> inMap;
    for (auto it = g.edges.begin(); it != g.edges.end(); ++it)
    {
        auto kv = *it;
